...
vector<Meeting> mergeCalendars(vector<Meeting> calendar1, vector<Meeting> calendar2) {
	...
	while (i < calendar1.size() && j < calendar2.size()) {
		Meeting meeting1 = calendar1[i], meeting2 = calendar2[j];
...
vector<Meeting> flattenCalendar(vector<Meeting> calendar) {
	...
	for (int i = 1; i < calendar.size(); i++) {
		Meeting currentMeeting = calendar[i], previousMeeting = flattened[flattened.size() - 1];
...
vector<StringMeeting> getMatchingAvailabilities(vector<Meeting> calendar, int meetingDuration) {
	...
	for(int i = i; i < calendar.size(); i++) {
		int start = calendar[i - 1].end, end = calendar[i].start, availabilityDuration = end - start;
	...
	for (int i = 0; i < matchingAvailabilities.size(); i++) {
		matchingAvailabilitiesInHours.push_back({minutesToTime(matchingAvailabilities[i].start), minutesToTime(matchingAvailabilities[i].end)});
	}
...
string minutesToTime(int minutes) {
	...
	string hoursString = to_string(hours), minutesString = mins < 10 ? "0" + to_string(mins) : to_string(mins);
