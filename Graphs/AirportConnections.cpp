...
void depthFirstTraverseAirports(unordered_map<string, AirportNode *> airportGraph, string airport, unordered_map<string, bool> *visitedAirports);
void markUnreachableConnections(unordered_map<string, AirportNode *> airportGraph, vector<AirportNode *> unreachableAirportNodes);
void deptFirstAddUnreachableConnections(unordered_map<string, AirportNode *> airportGraph, string airport, vector<string> *unreachableConnections, unordered_map<string, bool> *visitedAirports);
...
void markUnreachableConnections(unordered_map<string, AirportNode *> airportGraph, vector<AirportNode *> unreachableAirportNodes) {
	for (AirportNode *airportNode : unreachableAirportNodes) {
		...
		vector<string> unreachableConnections = {};
		unordered_map<string, bool> visitedAirports = {};
		deptFirstAddUnreachableConnections(airportGraph, airport, &unreachableConnections, &visitedAirports);
...
void deptFirstAddUnreachableConnections(unordered_map<string, AirportNode *> airportGraph, string airport, vector<string> *unreachableConnections, unordered_map<string, bool> *visitedAirports) {
	...
	vector<string> connections = airportGraph[airport]->connections;
	for (string connection : connections) {
		deptFirstAddUnreachableConnections (airportGraph, connection, unreachableConnections, visitedAirports);
	}
}

int getMinNumberOfNewConnections(unordered_map<string, AirportNode *> airportGraph, vector<AirportNode *> unreachableAirportNodes) {
	sort(unreachableAirportNodes.begin(), unreachableAirportNodes.end(),[](AirportNode *a1, AirportNode *a2) -> bool {
		return a2->unreachableConnections.size() < a1->unreachableConnections.size();
	});
